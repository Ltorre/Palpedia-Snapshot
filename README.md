...
The quickest-route workspace uses dedicated outlined cards for target selection and the final route. It offers clickable name suggestions as you type. If the target is already in the collection, the app clearly says so, then calculates the shortest route while deliberately excluding those copies. This keeps the route useful when breeding a new target to inherit a different set of passive traits. The selected male and female parents remain in dedicated cards above the scrollable Pal list.
